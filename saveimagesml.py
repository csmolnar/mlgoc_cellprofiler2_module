...
from cellprofiler.modules import saveimages as cpmsi
from mlgoc.objectsml import ObjectsML
...
IF_MOVIE       = "Movie"
IF_OBJECTS     = "Objects"
IF_ALL = [IF_IMAGE, IF_MASK, IF_OBJECTS]
...
class SaveImagesML(cpmsi.SaveImages):
    ...
    def create_settings(self):
        ...
        self.pathname = cpmsi.SaveImagesDirectoryPath(
    ...
    def save_objects(self, workspace):
        ...
        if filename is None:  # failed overwrite check
            return
        if isinstance(objects,ObjectsML):
            labels = objects.get_labels()
            if self.get_file_format() == FF_MAT:
                # pixels are saved as LxHxW matrix, where L is the number of phase field layers
                pixels = objects.get_labels()
    ...
    def upgrade_settings(self, setting_values, variable_revision_number,
        ...
        setting_values[OFFSET_DIRECTORY_PATH] = \
            cpmsi.SaveImagesDirectoryPath.upgrade_setting(setting_values[OFFSET_DIRECTORY_PATH])
