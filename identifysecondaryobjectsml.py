...
from cellprofiler.modules.identify import FI_IMAGE_SIZE
...
class IdentifySecondaryObjectsML(cpmi.Identify):
    ...
    def run(self, workspace):
        ...
        mask = image.mask
        inobjects = workspace.object_set.get_objects(self.primary_objects.value)
        ...
        if isinstance(inobjects, objectsml.ObjectsML):
            ...
            if self.method == M_DISTANCE_N:
                ...
                for l in range(labels_in.shape[0]):
                    distances,(i,j) = scind.distance_transform_edt(labels_in[l,:,:] == 0,
                                                                   return_indices = True)

                    dilate_mask = distances <= self.distance_to_dilate.value

                    labels_out[l,dilate_mask] =\
                        labels_in[l,i[dilate_mask],j[dilate_mask]]
            else:
            ...
            if self.wants_discard_edge and self.wants_discard_primary:
                ...
                lookup = scind.maximum(segmented_out,
                                       inobjects.segmented,
                                       range(np.max(inobjects.segmented)+1))
                lookup = fix(lookup)
                lookup[0] = 0
                lookup[lookup != 0] = np.arange(np.sum(lookup != 0)) + 1
                segmented_labels = lookup[inobjects.segmented]
                segmented_out = lookup[segmented_out]
                new_objects = cpo.Objects()
                new_objects.segmented = segmented_labels
                if inobjects.has_unedited_segmented:
                    new_objects.unedited_segmented = inobjects.unedited_segmented
                if inobjects.has_small_removed_segmented:
                    new_objects.small_removed_segmented = inobjects.small_removed_segmented
                new_objects.parent_image = inobjects.parent_image
            ...
            outobjects.segmented = segmented_out
            ...
            # Add measurements
            #
            object_count = len(np.unique(segmented_out))-1
            measurements = workspace.measurements
            cpmi.add_object_count_measurements(measurements, self.objects_name.value, object_count)
            add_object_location_measurements_ml(measurements, self.objects_name.value,
                                                  segmented_out)
            #
            ...
            if self.wants_discard_edge and self.wants_discard_primary:
                ...
                for parent_objects, parent_name, child_objects, child_name in (
                    (inobjects, self.primary_objects.value,
                     new_objects, self.new_primary_objects_name.value),
                    (new_objects, self.new_primary_objects_name.value,
                     outobjects, self.objects_name.value)):
                    children_per_parent, parents_of_children = \
            ...
            #
            labels_in = inobjects.unedited_segmented.copy()
            ...
            labels_in[(~ is_touching) & (inobjects.segmented == 0)] = 0
            #
            ...
            if self.method in (M_DISTANCE_B, M_DISTANCE_N):
                ...
                # output matrix that are missing from the segmented image
                #
                segmented_labels = inobjects.segmented
                segmented_out = self.filter_labels(small_removed_segmented_out,
                                                   inobjects, workspace)
            elif self.method == M_PROPAGATION:
            ...
            #
            children_per_parent, parents_of_children = \
                inobjects.relate_children(objects_out)
            measurements.add_measurement(self.primary_objects.value,
            ...
            if self.show_window:
                ...
                workspace.display_data.img = img
                workspace.display_data.segmented_out = segmented_out
                workspace.display_data.primary_labels = inobjects.segmented
                workspace.display_data.global_threshold = global_threshold
                workspace.display_data.object_count = object_count

    def display(self, workspace, figure):
        from cellprofiler.modules.identify import TS_BINARY_IMAGE
...
def create_contour_class_image(labels):
    labels = np.array( (labels > 0), dtype = float)
    ...
    if labels.ndim==2:
        ...
        l, h, w = labels.shape
        sum_layers = np.sum(labels,0)


    color_class_image = np.zeros((h,w,3))
    for i in range(l):
        color_class_image[:,:,0] = color_class_image[:,:,0] \
            + colors[i,0]*(labels[i,:,:]/sum_layers)
        color_class_image[:,:,1] = color_class_image[:,:,1] \
            + colors[i,1]*(labels[i,:,:]/sum_layers)
        color_class_image[:,:,2] = color_class_image[:,:,2] \
            + colors[i,2]*(labels[i,:,:]/sum_layers)
    color_class_image[np.isnan(color_class_image)] = 0.0
