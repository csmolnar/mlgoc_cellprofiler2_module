...
from identifysecondaryobjectsml import create_contour_class_image
...
class IdentifyTertiaryObjectsML(cpm.CPModule):

    module_name = "IdentifyTertiaryObjectsML"
    ...
    def run(self, workspace):
        ...
        #
        primary_objects = workspace.object_set.get_objects(self.primary_objects_name.value)

        #
        # Do the same with the secondary object
        secondary_objects = workspace.object_set.get_objects(self.secondary_objects_name.value)

        if type(primary_objects) != type(secondary_objects):
            raise NotImplementedError("Finding tertiary objects from different segmentation types is not yet supported (big: {}, small: {})"
                                      .format(type(primary_objects).__name__, type(secondary_objects).__name__))

        if isinstance(primary_objects, objectsml.ObjectsML):
            primary_labels = primary_objects.segmented.get_labels()
            secondary_labels = secondary_objects.segmented.get_labels()
            ...
            #
            primary_outline = np.array([outline(primary_labels[ll, :, :]) for ll in range(primary_labels.shape[0])])
            ...
            #
            tertiary_outlines = np.array([outline(tertiary_labels[ll,:,:])!=0 for ll in range(tertiary_labels.shape[0])])
            #
            # Make the tertiary objects container
            #
            tertiary_objects = objectsml.ObjectsML()
            ...
            #
            if self.use_outlines.value:
                outline_image = np.any(tertiary_outlines > 0, axis=0)
    ...
    def display(self, workspace, figure):
        if workspace.display_data.multi_layered:
            color_class_image_primary = create_contour_class_image(workspace.display_data.primary_labels)
            color_class_image_secondary = create_contour_class_image(workspace.display_data.secondary_labels)
            color_class_image_tertiary = create_contour_class_image(workspace.display_data.tertiary_labels)
            tertiary_outlines = workspace.display_data.tertiary_outlines
            outline_image = np.any(tertiary_outlines > 0, axis=0)

            figure.set_subplots((2, 2))
            figure.subplot_imshow_color(0, 0, color_class_image_primary, self.primary_objects_name.value)
            figure.subplot_imshow_color(1, 0, color_class_image_secondary, self.secondary_objects_name.value,
                                        sharexy=figure.subplot(0, 0))
            figure.subplot_imshow_color(0, 1, color_class_image_tertiary, self.subregion_objects_name.value,
                                        sharexy=figure.subplot(0, 0))
            figure.subplot_imshow_bw(1, 1, outline_image, "Outlines",
                                     sharexy=figure.subplot(0, 0))
        else:
...
IdentifyTertiarySubregionMLGOC = IdentifyTertiaryObjectsML
